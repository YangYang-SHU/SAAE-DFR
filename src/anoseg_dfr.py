...
class AnoSegDFR():
    ...
    def __init__(self, cfg):
        ...
        # feature extractor
        # self.extractor = VisionTransformer(image_size=(384,384),emb_dim=1024,mlp_dim=4096,num_heads=16,num_layers=self.n_layers).to(self.device)         
        self.extractor = VisionTransformer(image_size=(384,384),emb_dim=768,mlp_dim=3072,num_heads=12,num_layers=self.n_layers).to(self.device)
        self.extractor = load_pretrained_vit(self.extractor)
        ...
        self.eval_data_loader = DataLoader(self.train_data, batch_size=16, shuffle=False, num_workers=0)
    ...
    def build_classifier(self):
        ...
        if self.n_dim is None:
            ...
            for i, normal_img in enumerate(self.eval_data_loader):
                ...
                feat = self.extractor(normal_img)
                feat = self.extractor.feat_vec(feat)
        ...
        autoencoder = SAAE(
                            ...
                            depth = 1,
                            heads = 3,
                            mlp_dim = 2 * self.n_dim,
                            in_channels=in_feat,
                            latent_dim=self.n_dim,
                            is_bn=self.cfg.is_bn,
                            channels = self.n_dim,
                            dropout = 0.0,
                            emb_dropout = 0.0
                        ).to(self.device)
        model_name = "AnoSegDFR({})_l{}_d{}_{}".format('BN' if self.cfg.is_bn else 'noBN',
                                                                self.n_layers,
                                                                self.n_dim,  
                                                                self.cfg.upsample)
    ...
    def build_dataset(self, is_train):
        ...
        if is_train:
            dataset = NormalDataset(normal_data_path, normalize=False)
        else:
            dataset = TestDataset(path=abnormal_data_path,normalize=False)
        return dataset
...
#             print("Done.")
    ...
    def segment_evaluation(self):
        ...
        for i, (img, mask, name) in enumerate(self.test_data_loader):    # batch size is 1.
            ...
            #     ax1.imshow(resize(mask[0], (384, 384)))
            #     ax1.set_title("gt")
        ...
        for step in range(max_step):
            ...
            for i in range(len(binary_score_maps)):    # for i th image
                ...
                for prop in props:
                    ...
                    cropped_mask = prop.filled_image #masks[i][x_min:x_max, y_min:y_max]
